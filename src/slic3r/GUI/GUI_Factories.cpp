...
wxMenu* MenuFactory::append_submenu_add_handy_model(wxMenu* menu, ModelVolumeType type) {
    ...
    for (auto &item : {L("Elegoo Cube"), L("3DBenchy"), L("Autodesk FDM Test"),
                       L("Voron Cube"), L("Stanford Bunny"), L("Elegoo String Hell") }) {
        append_menu_item(
            ...
            [type, item](wxCommandEvent&) {
                ...
                else if (file_name == L("Elegoo String Hell")) {
                    file_name     = "Elegoo_stringhell.3mf";
                    is_stringhell = true;
                } else
                    return;
